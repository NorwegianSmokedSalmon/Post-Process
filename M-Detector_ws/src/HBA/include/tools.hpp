...
void downsample_voxel_ckx(pcl::PointCloud<PointType>& pc, double voxel_size)
{
  // Convert PointCloud to PCLPointCloud2
  pcl::PCLPointCloud2::Ptr tmpcloud(new pcl::PCLPointCloud2());
  pcl::PCLPointCloud2::Ptr cloud_filtered(new pcl::PCLPointCloud2());
  
  pcl::toPCLPointCloud2(pc, *tmpcloud);
  
  // Apply VoxelGrid filter
  pcl::VoxelGrid<pcl::PCLPointCloud2> voxel_filter;
  voxel_filter.setInputCloud(tmpcloud);
  
  float leaf_size = static_cast<float>(voxel_size);
  voxel_filter.setLeafSize(leaf_size, leaf_size, leaf_size);
  
  voxel_filter.filter(*cloud_filtered);
  
  // Convert filtered PCLPointCloud2 back to PointCloud
  pcl::PointCloud<PointType> cloud_filtered_pc;
  pcl::fromPCLPointCloud2(*cloud_filtered, cloud_filtered_pc);
  
  // Update the original point cloud
  pc = cloud_filtered_pc;
}
